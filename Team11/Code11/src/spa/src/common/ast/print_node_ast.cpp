...
auto PrintNode::populate_pkb_entities(const std::shared_ptr<pkb::WriteFacade>& write_facade) const -> void {
    write_facade->add_statement(std::to_string(get_statement_number()), StatementType::Print);
}

auto PrintNode::populate_pkb_uses(const std::shared_ptr<pkb::WriteFacade>& write_facade,
