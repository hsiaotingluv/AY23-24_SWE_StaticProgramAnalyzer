...
class ModifiesSEvaluator : public ClauseEvaluator {
    ...
    // e.g. Modifies(s1, "v")
    auto eval_modifies_s(const std::shared_ptr<qps::StmtSynonym>& stmt_synonym, const qps::QuotedIdent& identifier)
        -> std::optional<Table>;

    // e.g. Modifies(s1, _)
    auto eval_modifies_s(const std::shared_ptr<qps::StmtSynonym>& stmt_synonym, const qps::WildCard&)
        -> std::optional<Table>;

    // e.g. Modifies(3, v)
    auto eval_modifies_s(const qps::Integer& stmt_num, const std::shared_ptr<qps::VarSynonym>& var_syn)
        -> std::optional<Table>;

    // e.g. Modifies(3, "v")
    auto eval_modifies_s(const qps::Integer& stmt_num, const qps::QuotedIdent& identifier) -> std::optional<Table>;

    // e.g. Modifies(3, _)
    auto eval_modifies_s(const qps::Integer& stmt_num, const qps::WildCard&) -> std::optional<Table>;
