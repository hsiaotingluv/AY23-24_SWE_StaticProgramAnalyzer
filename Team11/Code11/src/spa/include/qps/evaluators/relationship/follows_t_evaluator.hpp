...
class FollowsTEvaluator : public ClauseEvaluator {
    ...
    // e.g. Follows*(3, _)
    [[nodiscard]] auto eval_follows_t(const qps::Integer& stmt_num_1, const qps::WildCard&) const
        -> std::optional<Table>;
    ...
    // e.g. Follows*(_, 3)
    [[nodiscard]] auto eval_follows_t(const qps::WildCard&, const qps::Integer& stmt_num_2) const
        -> std::optional<Table>;

    // e.g. Follows*(_, _)
    [[nodiscard]] auto eval_follows_t(const qps::WildCard&, const qps::WildCard&) const -> std::optional<Table>;
