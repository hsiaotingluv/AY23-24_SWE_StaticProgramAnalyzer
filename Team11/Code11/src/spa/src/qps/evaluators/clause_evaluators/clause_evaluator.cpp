...
auto qps::ClauseEvaluator::negate_result(qps::OutputTable output_table) const -> qps::OutputTable {
    return std::visit(overloaded{
        [](const UnitTable&) -> qps::OutputTable {
            return Table{};
        },
        [this](const Table &table) -> qps::OutputTable {
            if (is_empty(table)) return UnitTable{};
            auto column_synonyms = table.get_column();
            // [EXPENSIVE] for each synonym, do a synonym.scan(read_facade) and put into a table. cross join all these tables.
