...
auto CfgBuilder::create_empty_cfg(std::string const& proc_name) -> std::shared_ptr<Cfg> {
    const auto cfg = std::make_shared<Cfg>();
    proc_map.insert(std::make_pair(proc_name, cfg)); // Add the Cfg to the ProcMap.
    return cfg;
};

auto CfgBuilder::populate_stmt_num_map_by_node(StatementNumbers const& stmt_nums, std::string const& proc_name,
...
auto CfgBuilder::populate_stmt_num_map_by_procedure(std::string const& proc_name, Graph const& graph) -> Graph {
...
auto CfgBuilder::build(std::shared_ptr<AstNode> const& ast) -> ProcMap {
...
auto operator<<(std::ostream& os, CfgBuilder const& cfg_builder) -> std::ostream& {
