...
class PatternAssignEvaluator : public ClauseEvaluator {
    ...
    // e.g. pattern a(x, _)
    [[nodiscard]] auto eval_pattern_assign(const std::shared_ptr<Synonym>& synonym, const qps::WildCard &) const -> OutputTable;

    // e.g. pattern a("x", _"v"_)
    [[nodiscard]] auto eval_pattern_assign(const qps::QuotedIdent &quoted_ident, const qps::PartialMatch &partial_match) const
        -> OutputTable;

    // e.g. pattern a("x", _)
    [[nodiscard]] auto eval_pattern_assign(const qps::QuotedIdent &quoted_ident, const qps::WildCard &) const -> OutputTable;
    ...
    // e.g. pattern a("x", "v")
    [[nodiscard]] auto eval_pattern_assign(const QuotedIdent& quoted_ident, const ExactMatch& exact) const -> OutputTable;
