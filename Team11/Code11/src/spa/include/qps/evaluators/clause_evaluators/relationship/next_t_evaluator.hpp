...
class NextTEvaluator : public ClauseEvaluator {
  ...
  public:
    NextTEvaluator(std::shared_ptr<pkb::ReadFacade> read_facade, NextT next_t, bool is_negated)
        : ClauseEvaluator(std::move(read_facade), is_negated), next_t(std::move(next_t)) {
    }
