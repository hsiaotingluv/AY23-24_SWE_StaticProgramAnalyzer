...
auto UsesSEvaluator::eval_uses_s(const std::shared_ptr<ReadFacade>& read_facade) {
    return overloaded {
            ...
            // e.g. Uses(a/pn/s, v)
            [read_facade](const std::shared_ptr<StmtSynonym>& stmt_synonym, const std::shared_ptr<VarSynonym>& var_synonym) -> std::optional<Table> {
            ...
            // e.g. Uses(a/pn/s, "v")
            [read_facade](const std::shared_ptr<StmtSynonym>& stmt_synonym, const qps::QuotedIdent& quoted_ident) -> std::optional<Table> {
            ...
            // e.g. Uses(a/pn/s, _)
            [read_facade](const std::shared_ptr<StmtSynonym>& stmt_synonym, const qps::WildCard& wild_card) -> std::optional<Table> {
                auto table = Table{{stmt_synonym}};
                const auto relevant_stmts = scan_entities(read_facade, stmt_synonym);
            ...
            // e.g. Uses(3, v)
            [read_facade](const qps::Integer& stmt_num, const std::shared_ptr<VarSynonym>& var_synonym) -> std::optional<Table> {
