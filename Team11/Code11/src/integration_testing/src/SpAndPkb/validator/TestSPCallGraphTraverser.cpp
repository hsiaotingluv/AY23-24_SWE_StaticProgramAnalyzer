...
TEST_CASE("Test SP Call Graph Traverser - Simple Usability Tests") {
    ...
    const sp::SemanticValidator::CallGraph ONE_TO_NONE_CALLGRAPH = {
        {"Procedure1", ProcNames()},
        {"Procedure2", ProcNames()}
    };
    const sp::SemanticValidator::CallGraph ONE_TO_ONE_CALLGRAPH = {
        {"Procedure1", ProcNames({"Procedure2"})},
    ...
    SECTION("Test Traversing a CallGraph with Multiple Callers - Success") {
        ...
        REQUIRE(read_facade->get_all_calls_callers() == ProcNames({"Procedure2", "Procedure3", "Procedure4", "Procedure5", "Procedure6"}));
        REQUIRE(read_facade->has_calls_relation("Procedure2", "Procedure1"));
