...
class ParentEvaluator : public ClauseEvaluator {
    ...
    // e.g. Parent(3, 4)
    [[nodiscard]] auto eval_parent(const qps::Integer& stmt_num_1, const qps::Integer& stmt_num_2) const
        -> std::optional<Table>;

    // e.g. Parent(3, _)
    [[nodiscard]] auto eval_parent(const qps::Integer& stmt_num_1, const qps::WildCard&) const -> std::optional<Table>;
    ...
    // e.g. Parent(_, 3)
    [[nodiscard]] auto eval_parent(const qps::WildCard&, const qps::Integer& stmt_num_2) const -> std::optional<Table>;

    // e.g. Parent(_, _)
    [[nodiscard]] auto eval_parent(const qps::WildCard&, const qps::WildCard&) const -> std::optional<Table>;
