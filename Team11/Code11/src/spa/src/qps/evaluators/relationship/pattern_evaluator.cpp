...
auto PatternEvaluator::evaluate() const -> OutputTable {
...
auto PatternEvaluator::eval_pattern(const std::shared_ptr<Synonym>& synonym, const WildCard&) const -> OutputTable {
    auto table = Table{{pattern.assign_synonym, synonym}};
...
auto PatternEvaluator::eval_pattern(const QuotedIdent& quoted_ident, const WildCard&) const -> OutputTable {
    auto table = Table{{pattern.assign_synonym}};
...
auto PatternEvaluator::eval_pattern(const WildCard&, const PartialMatch& partial_match) const -> OutputTable {
    auto table = Table{{pattern.assign_synonym}};
    const auto all_partial_matches = read_facade->get_all_assignments_rhs_partial(partial_match.expr.value);
...
auto PatternEvaluator::eval_pattern(const WildCard&, const WildCard&) const -> OutputTable {
    auto table = Table{{pattern.assign_synonym}};
...
auto PatternEvaluator::eval_pattern(const WildCard&, const ExactMatch& exact) const -> OutputTable {
...
auto PatternEvaluator::eval_pattern(const QuotedIdent& quoted_ident, const ExactMatch& exact) const -> OutputTable {
