...
auto PatternEvaluator::select_eval_method() {
    return overloaded{
        ...
        // e.g. pattern a("x", _"v"_)
        [this](const qps::QuotedIdent& quoted_ident,
               const qps::PartialMatch& partial_match) -> std::optional<Table> {
        ...
        [this](const qps::WildCard& wild_card,
               const qps::PartialMatch& partial_match) -> std::optional<Table> {
        ...
        // e.g. pattern a("x", "v")
        [this](const QuotedIdent& quoted_ident,
               const ExactMatch& exact) -> std::optional<Table> {
...
auto PatternEvaluator::evaluate() -> std::optional<Table> {
    ...
    auto PatternEvaluator::eval_pattern(const std::shared_ptr<Synonym> &synonym,
                                        const PartialMatch &partial_match) -> std::optional<Table> {
    ...
    auto PatternEvaluator::eval_pattern(const std::shared_ptr<Synonym> &synonym,
                                        const WildCard &wild_card) -> std::optional<Table> {
        auto table = Table{{pattern.assign_synonym, synonym}};
    ...
    auto PatternEvaluator::eval_pattern(const QuotedIdent &quoted_ident,
                                        const PartialMatch &partial_match) -> std::optional<Table> {
    ...
    auto
    PatternEvaluator::eval_pattern(const QuotedIdent &quoted_ident, const WildCard &wild_card) -> std::optional<Table> {
        auto table = Table{{pattern.assign_synonym}};
    ...
    auto PatternEvaluator::eval_pattern(const WildCard &wild_card,
                                        const PartialMatch &partial_match) -> std::optional<Table> {
        auto table = Table{{pattern.assign_synonym}};
        const auto all_partial_matches = read_facade->get_all_assignments_rhs_partial(partial_match.expr.value);
    ...
    auto
    PatternEvaluator::eval_pattern(const WildCard &wild_card_1, const WildCard &wild_card_2) -> std::optional<Table> {
        auto table = Table{{pattern.assign_synonym}};
